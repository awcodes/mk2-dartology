<x-base-layout>
    ...
    @if ($routines)
        <ul class="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            ...
            <li>
                <a href="{{ route('routines.show', $routine) }}" class="block h-full p-5 border border-gray-800 rounded shadow-md reset-link group hover:bg-gray-800 focus:bg-gray-800">
                        <h2 class="inline-block group-hover:text-secondary-500">{{ $routine->name }}</h2>
                        <p class="leading-6 group-hover:text-secondary-300">
                            {!! $routine->goal !!}
                        </p>
                        <p class="-mt-2 text-sm text-gray-400"><strong>Skills:</strong> {{ $routine->skills->implode('name', ', ') }}</p>
                </a>
            </li>
            @endforeach
        </ul>
